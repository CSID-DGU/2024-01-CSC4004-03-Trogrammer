...
public static class Database {
    ...
    public readonly static List<Restaurant> restaurant = new List<Restaurant>{
        ...
        new Restaurant("필동함박 충무로 본점", new List<Menuset>{
            new Menuset("까르보파스타", 11500.0f),
			new Menuset("투움바파스타", 11500.0f),
        }),
        new Restaurant("홍짜장 동국대점", new List<Menuset>{
            new Menuset("짜장면", 5500.0f),
			new Menuset("짬뽕", 6500.0f),
        }),

        new Restaurant("평강삼계탕 장충점", new List<Menuset>{
            new Menuset("삼계탕", 17000.0f),
        }),
		new Restaurant("일미정", new List<Menuset>{
            new Menuset("불고기", 8000.0f),
			new Menuset("된장찌개", 9000.0f),
        }),
        new Restaurant("필동반점", new List<Menuset>{
            new Menuset("짬뽕", 7000.0f),
            new Menuset("짜장면", 6000.0f),
        }),
        new Restaurant("스시노칸도 충무로역점", new List<Menuset>{
            new Menuset("초밥", 22000.0f),
        }),
		new Restaurant("스담", new List<Menuset>{
            new Menuset("초밥", 18000.0f),
        }),
		new Restaurant("낙원의소바", new List<Menuset>{
            new Menuset("돈까스", 15000.0f),
			new Menuset("로스카츠", 13000.0f),
        }),
		new Restaurant("서울카츠 충무로본점", new List<Menuset>{
            new Menuset("등심 카츠", 14000.0f),
			new Menuset("안심 카츠", 15000.0f),
        }),
		new Restaurant("모리짱", new List<Menuset>{
            new Menuset("돈고츠라멘", 8000.0f),
			new Menuset("소유라멘", 7000.0f),
        }),
        new Restaurant("도치피자 장충점", new List<Menuset>{
            new Menuset("디아볼라 피자", 24000.0f),
            new Menuset("고르곤졸라 피자",22000.0f),
        }),
		new Restaurant("미스터피자 충무로점", new List<Menuset>{
            new Menuset("포테이토골드 피자", 31500.0f),
            new Menuset("페퍼로니 피자",20500.0f),
			new Menuset("콤비네이션 피자",21500.0f),
        }),
    };
}
